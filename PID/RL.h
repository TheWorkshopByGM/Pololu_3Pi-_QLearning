...
class RL_c{
  ...
  int current_error = 0;
  int prev_error = 0;
  unsigned long prev_error_ts = 0;

  //PID
  float proportional = 0;
  float derivative = 0;
  float integral = 0;
  float integral_rate = 0.75;
  int Kp = 35;
  int Ki = 1;
  int Kd = 15;
  ...
  void PID_control(){
    ...
    //PID
    proportional = (float)current_error/100.0;
    integral = (float)current_error/10.0 + integral_rate*integral;
    derivative = (float)((float)current_error - (float)prev_error)/(float)((millis()-prev_error_ts));
    prev_error_ts = millis();
    prev_error = current_error;

    //Run Motors
    int PID_Speed = (int)((float)Kp*(float)proportional + (float)Ki*(float)integral + (float)Kd*(float)derivative);
    motors.Run(fwd_Bias-PID_Speed,fwd_Bias+PID_Speed);
  }
